...
#load the data
st=(dt.today()-datetime.timedelta(days=100)).strftime("%d-%m-%Y")
en=dt.today().strftime("%d-%m-%Y")
df=equity_history('SBIN','EQ',st, en)
...
fig.add_trace(
go.Scatter(
    x=dates,  # np.array(te11).flatten(), 
    y=rbf_svr.predict(output_days),
    mode='lines+markers',
    name='data'))
fig.update_layout(
title="Predicted Close Price of next " + str(11 - 1) + " days",
xaxis_title="Date",
